...
module.exports.run = async (client, message, args, mentionFix, getUser) => {
  ...
  const database = await server
    .findOne({ guildId: message.guild.id })
    .catch((err) => console.log(err));

  if (!category) {
    return message.channel.send(
      "Tracker has not been setup, use the 'setup' command."
    );
  }
  ...
  if (args[0].toLowerCase() === "add") {
    ...
    let channel = message.guild.channels.cache.find(
      (c) =>
        c.id ===
        database.config.userspawn.find(
    ...
    console.log(
      `[TRACKER] ${message.guild.name} (${message.guild.id}) is now tracking ${args[1]}.`
    );
    ...
    if (tracker.size === 0) {
      ...
      tracker
        .filter((v) => v.name === "tracker")
        .forEach(async (v) => await v.delete());
    ...
    console.log(
      `[TRACKER] ${message.guild.name} (${message.guild.id}) is no longer tracking ${args[1]}.`
    );
  } else {
    message.channel.send(
      "Invalid type, please use 'add' or 'remove' then the username."
    );
  }
};
...
module.exports.config = {
  ...
  aliases: ["track"],
  usages: ["tracker <add|remove> <username>"],
  examples: ["tracker add calculatlons", "tracker remove calculatlons"],
};
