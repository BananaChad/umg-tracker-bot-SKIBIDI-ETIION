...
there will be more commands in the future.. haha..\`\`\``;
  ...
  const cmd =
    data.commands.get(args[0].toLowerCase()) ||
    data.commands.get(data.alias.get(args[0].toLowerCase()));
...
### **${cmd.config.name} command**
  ...
  - Aliases: ${
    !cmd.config.aliases.length
      ? "None"
      : cmd.config.aliases.map((r) => `"${r}"`).join(", ")
  }
...
\`\`\``;
  ...
  if (
    !data.commands.filter((r) => r.config.cmd === args[0].toLowerCase()).size
  ) {
